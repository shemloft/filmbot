...
public class DialogTest extends TestCase {
	...
	public void testNoNextFilmYear() {
		...
		Assert.assertEquals("Все фильмы этого года, имеющиеся в базе, были предоставлены", dialog.processInput("/next"));
	}
	
	public void testNoNextFilmCountry() {
		Assert.assertEquals("Достучаться до небес", dialog.processInput("/c Германия"));
		Assert.assertEquals("Все фильмы этой страны, имеющиеся в базе, были предоставлены", dialog.processInput("/next"));
	}
	
	public void testsUserSeenFilmCountry() {
		Assert.assertEquals("Все фильмы этой страны, имеющиеся в базе, были предоставлены", dialog.processInput("/c Франция"));
	}
